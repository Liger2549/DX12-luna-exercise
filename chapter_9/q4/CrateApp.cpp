...
void CrateApp::AnimateMaterials(const GameTimer& gt)
{
    ...
    auto flareMat = matLib["flare"];

    static float angle = 0.0f;
    angle -= 1.0f * gt.DeltaTime();

    XMMATRIX rotate = XMMatrixRotationZ(angle);
    XMMATRIX fromCenter = XMMatrixTranslation(0.5f, 0.5f, 0.0f);
    XMMATRIX toCenter = XMMatrixTranslation(-0.5f, -0.5f, 0.0f);
    XMMATRIX finalTransform = toCenter * rotate * fromCenter;

    XMStoreFloat4x4(&flareMat->MatTransform, finalTransform);
...
void CrateApp::BuildRenderItems()
{
    ...
    AddRenderItem(RenderLayer::Opaque, worldTransform, texTransform, matLib["flare"], mGeometries["shapeGeo"].get(), mGeometries["shapeGeo"]->DrawArgs["box"]);
}
