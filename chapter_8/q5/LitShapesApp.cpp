...
void LitShapesApp::UpdateMainPassCB(const GameTimer& gt)
{
    ...
    mMainPassCB.gNumDirLights = 0;
    mMainPassCB.gNumPointLights = 0;
    mMainPassCB.gNumSpotLights = 10;

    for (int i = 0; i < 5; ++i)
    {
        mMainPassCB.gLights[i * 2].Strength = mLightStrength;
        mMainPassCB.gLights[i * 2].FalloffStart = 0.0f;
        mMainPassCB.gLights[i * 2].FalloffEnd = 12.0f;
        mMainPassCB.gLights[i * 2].SpotPower = 2.0f;
        mMainPassCB.gLights[i * 2].Direction = XMFLOAT3(0.0f, -1.0f, 0.0f);
        mMainPassCB.gLights[i * 2].Position = XMFLOAT3(-5.0f, 5.0f, -10.0f + i * 5.0f);

        mMainPassCB.gLights[i * 2 + 1].Strength = mLightStrength;
        mMainPassCB.gLights[i * 2 + 1].FalloffStart = 0.0f;
        mMainPassCB.gLights[i * 2 + 1].FalloffEnd = 12.0f;
        mMainPassCB.gLights[i * 2 + 1].SpotPower = 2.0f;
        mMainPassCB.gLights[i * 2 + 1].Direction = XMFLOAT3(0.0f, -1.0f, 0.0f);
        mMainPassCB.gLights[i * 2 + 1].Position = XMFLOAT3(+5.0f, 5.0f, -10.0f + i * 5.0f);
    }
...
#define COMMA_DEBUG_ARGS
    ...
    std::vector<LPCWSTR> vsArgs = std::vector<LPCWSTR> { L"-E", L"VS", L"-T", L"vs_6_6", L"-I", L"Shaders" COMMA_DEBUG_ARGS };
    std::vector<LPCWSTR> psArgs = std::vector<LPCWSTR> { L"-E", L"PS", L"-T", L"ps_6_6", L"-I", L"Shaders" COMMA_DEBUG_ARGS };
