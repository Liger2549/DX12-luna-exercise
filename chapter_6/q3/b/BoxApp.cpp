...
void BoxApp::Draw(const GameTimer& gt)
{
	...
	// We are not using an index buffer for this sample, but if we were, we would set it here.
    // mCommandList->IASetIndexBuffer(&mPoint->IndexBufferView());
    mCommandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_LINESTRIP);

    mCommandList->DrawInstanced(mGeo->DrawArgs["line"].VertexCount, 1, 0, 0);
...
{
    Vertex v1 = { XMFLOAT3(-0.9f, -0.2f, 0.0f), XMFLOAT4(Colors::White) };
	Vertex v2 = { XMFLOAT3(-0.6f,  0.3f, 0.0f), XMFLOAT4(Colors::White) };
	Vertex v3 = { XMFLOAT3(-0.3f, -0.1f, 0.0f), XMFLOAT4(Colors::White) };
	Vertex v4 = { XMFLOAT3(-0.1f,  0.5f, 0.0f), XMFLOAT4(Colors::White) };
	Vertex v5 = { XMFLOAT3(0.1f,  0.2f, 0.0f), XMFLOAT4(Colors::White) };
	Vertex v6 = { XMFLOAT3(0.3f,  0.6f, 0.0f), XMFLOAT4(Colors::White) };
	Vertex v7 = { XMFLOAT3(0.5f,  0.4f, 0.0f), XMFLOAT4(Colors::White) };
	Vertex v8 = { XMFLOAT3(0.8f,  0.7f, 0.0f), XMFLOAT4(Colors::White) };
    ...
    const UINT vbVByteSize = (UINT)vertices.size() * sizeof(Vertex);

	mGeo = std::make_unique<MeshGeometry>();
	mGeo->Name = "line";

    mGeo->VertexBufferCPU.resize(vbVByteSize);
	CopyMemory(mGeo->VertexBufferCPU.data(), vertices.data(), vbVByteSize);
	...
	mGeo->VertexBufferByteSize = vbVByteSize;
