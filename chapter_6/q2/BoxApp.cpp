...
void BoxApp::Draw(const GameTimer& gt)
{
    ...
    D3D12_VERTEX_BUFFER_VIEW vertexBuffers[2] = { mBoxGeo->VertexBufferView(), mBoxGeo->VertexBufferView2() };
    mCommandList->IASetVertexBuffers(0, 2, vertexBuffers);
...
void BoxApp::BuildBoxGeometry(ID3D12Device* device, DirectX::ResourceUploadBatch& uploadBatch)
{
    ...
    const UINT vbPosByteSize = (UINT)positions.size() * sizeof(VPosData);
    const UINT vbColorByteSize = (UINT)colors.size() * sizeof(VColorData);
    const UINT ibByteSize = (UINT)indices.size() * sizeof(uint16_t);

	mBoxGeo = std::make_unique<MeshGeometryEx>();
	mBoxGeo->Name = "boxGeo";

	// Position vertex buffer.
    mBoxGeo->VertexBufferCPU.resize(vbPosByteSize);
	CopyMemory(mBoxGeo->VertexBufferCPU.data(), positions.data(), vbPosByteSize);

	CreateStaticBuffer(
		device, uploadBatch,
		positions.data(), positions.size(), sizeof(VPosData), 
	...
	// Color vertex buffer.
	mBoxGeo->VertexBufferCPU2.resize(vbColorByteSize);
	CopyMemory(mBoxGeo->VertexBufferCPU2.data(), colors.data(), vbColorByteSize);

    CreateStaticBuffer(
        device, uploadBatch,
        colors.data(), colors.size(), sizeof(VColorData),
        D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER,
        &mBoxGeo->VertexBufferGPU2);
	mBoxGeo->VertexByteStirde2 = sizeof(VColorData);
	mBoxGeo->VertexBufferByteSize2 = vbColorByteSize;
	...
	submesh.VertexCount = (UINT)positions.size();
